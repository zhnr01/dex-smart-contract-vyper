...
def getTokenAmount(_ethSold: uint256) -> uint256:
    ...
    return Exchange(self).getAmount(
        _ethSold,
        self.balance,
        tokenReserve
    )
...
def getEthAmount(_tokenSold: uint256) -> uint256:
    ...
    tokenReserve: uint256 = self.getReserve()
...
def ethToTokenSwap(_minTokens: uint256):
    ...
    tokensBought: uint256 = self.getAmount(
...
def tokenToEthSwap(_tokensSold: uint256, _minEth: uint256):
    ...
    ethBought: uint256 = self.getAmount(
    ...
    IERC20(self.tokenAddress).transferFrom(
